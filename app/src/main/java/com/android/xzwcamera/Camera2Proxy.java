...
public class Camera2Proxy {
    ...
    private String mNextVideoAbsolutePath;//视频保存位置
    ...
    private int CONTROL_AE_MODE = CaptureRequest.CONTROL_AE_MODE_OFF;


    private MediaScannerConnection mMediaScannerConnection;//扫描相册视频

    private int mDisplayRotate = 0;
    private int mDeviceOrientation = 0; // 设备方向，由相机传感器获取
    private int mZoom = 1; // 缩放

    private static boolean isKeepFlash = false;

    /*Face detected*/
    private int mFaceDetectMode = CaptureRequest.STATISTICS_FACE_DETECT_MODE_SIMPLE;
    ...
    private Camera.FaceDetectionListener mFaceDetectListener;
    ...
    public void openCamera() {
        ...
        try {
            ...
            mDeviceOrientation = characteristics.get(CameraCharacteristics.SENSOR_ORIENTATION);
    ...
    /*private void initFaceDetect() {
        ...
        int scaledWidth = mPreviewSize.getWidth() / activeArraySizeRect.width();
        int scaledHeight = mPreviewSize.getHeight() / activeArraySizeRect.height();
        boolean mirror = mCameraId == CameraCharacteristics.LENS_FACING_FRONT;
        mFaceDetectMatrix.setRotate(getJpegOrientation(90));
    }*/
    ...
    public void startPreview() {
        ...
        Log.v(TAG, "isKeepFlash : " + isKeepFlash);
        if(isKeepFlash) {
    ...
    public void setupMediaRecorder(){
        ...
        Log.d(TAG,"VideoFilePath："+mNextVideoAbsolutePath);
        ...
        mMediaRecorder.setOrientationHint(getJpegOrientation(rotation));
        try {
    ...
    public void startRecordingVideo(final Camera2TextureView mCameraView){
        try{
            stopPreview();
            //添加预览的Surface
            mPreviewSurfaceTexture = mCameraView.getSurfaceTexture();
            assert mPreviewSurfaceTexture != null;
            ...
            Log.d(TAG,"PreviewSurface: " + mPreviewSurface);
            mPreviewRequestBuilder.addTarget(mPreviewSurface);

            mRecordSurface = mMediaRecorder.getSurface();
            surfaces.add(mRecordSurface);
            Log.d(TAG,"RecordSurface: " + mRecordSurface);
            mPreviewRequestBuilder.addTarget(mRecordSurface);
            Log.d(TAG,"start recording isKeepFlash: " + isKeepFlash);
            if(isKeepFlash) {
                mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AE_MODE, CaptureRequest.CONTROL_AE_MODE_ON);
                mPreviewRequestBuilder.set(CaptureRequest.FLASH_MODE, CaptureRequest.FLASH_MODE_TORCH);
            }
    ...
    public String getVideoPath(){
        mVideoAbsolutePath = mNextVideoAbsolutePath;
        return mVideoAbsolutePath;
    }
    ...
    public void stopRecording(){
        ...
        getVideoPath();
        Log.d(TAG,"NextVideoAbsolutePath : " + mNextVideoAbsolutePath);
        String videoThumbnailPath = mNextVideoAbsolutePath;
    ...
    private void scanFile(String filePath) {
        ...
        mMediaScannerConnection = new MediaScannerConnection(mActivity, null);
        //调用connect
    ...
    public void setFlashMode(String flashMode) {
        ...
        try{
            mCaptureSession.setRepeatingRequest(mPreviewRequestBuilder.build(),mAfCaptureCallback,mBackgroundHandler);
        } catch (CameraAccessException e) {
...
//            handleFaces(result);
    ...
    /*private void handleFaces(TotalCaptureResult result) {
        ...
        for (int i = 0; i < faces.length; i++) {
            ...
            RectF resultFaceRect = new RectF();
            if(mCameraId == CaptureRequest.LENS_FACING_FRONT){
                resultFaceRect = rawFaceRect;
            }else {
                resultFaceRect = new RectF(rawFaceRect.left, rawFaceRect.top - mPreviewSize.getHeight(), rawFaceRect.right, rawFaceRect.bottom - mPreviewSize.getWidth());
            }
